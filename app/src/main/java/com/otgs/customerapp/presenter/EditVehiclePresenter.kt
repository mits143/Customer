...
class EditVehiclePresenter : EditVehicleView.MainPresenter {

    var context: Context? = null
    var mainView: EditVehicleView.MainView? = null

    @NonNull
    var disposable: Disposable? = null

    constructor(context: Context?, mainView: EditVehicleView.MainView?) {
    ...
    override fun loadVehicleDetailsData(
        token: String,
        user_id: String,
        user_type: String,
        ...
        if (NetWorkConection.isNEtworkConnected(context!!)) {
            disposable = ApiClient.instance
                ...
                .subscribe({ listResponse ->
                    ...
                    when (responseCode) {
                        200, 201, 202, 204 -> {
                            mainView!!.onSuccessVehicleDetail(listResponse)
                        }
                        400, 401, 500 -> {
                            mainView!!.onError(responseCode)
                        }
